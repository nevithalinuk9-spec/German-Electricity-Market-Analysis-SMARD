...
# run the notebook
jupyter lab   # open notebooks/SMARD_analysis.ipynb
```

There is no lint config, formatter, or test suite in this repo — don't invent
commands for them.
...
**Architecture: importable `.py` modules in `src/`, orchestrated from a single
notebook.** The notebook is the only file that runs. Modules provide functions;
the notebook calls them, displays results, and holds all written interpretation.

Python on Windows. pandas, numpy, scikit-learn, matplotlib/seaborn/plotly, shap.
...
notebooks/
  SMARD_analysis.ipynb   THE ORCHESTRATOR — the only file that is run
...
**Phase 5 — ML.** `models.py` + `evaluate.py`. Primary: binary classifier for
negative-price hours. Secondary: GBM on actuals with SHAP, framed as attribution.
...
Secondary: GBM on actuals with SHAP, framed as attribution.
...
| model | MAE | R² |
|---|---|---|
| naive `price_lag24` | 25.97 | — |
| GBM, lags + calendar only | 26.56 | 0.455 |
| GBM, actual generation | 20.20 | 0.704 |
| Ridge, actual generation | 20.72 | 0.711 |

The lags-only model **loses to naive**. That is a real result, not a bug. Do not
